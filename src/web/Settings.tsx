...
const SettingsSetup = () => {
  ...
  // --- FORM & TARGET STATES ---
  const [deleteTarget, setDeleteTarget] = useState<{type: 'cert' | 'staff', item: any} | null>(null);
  ...
  const handleCertSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCertificates(prev => isEditMode ? prev.map(c => c.id === result.id ? result : c) : [...prev, result]);
  ...
  const openStaffModal = (staff?: Staff) => {
    ...
    setStaffFormData(staff 
        ? { ...staff, password: '', password_confirmation: '' } 
        : { id: 0, name: '', email: '', role: 'Medical Staff', username: '', password: '', password_confirmation: '' }
    );
  ...
  const handleStaffSubmit = async (e: React.FormEvent) => {
    ...
    // Cleanup for Edit Mode if passwords are empty
    ...
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        // This logic automatically updates the table state
        setStaffList(prev => isEditMode 
  ...
  // --- DELETE LOGIC ---
  const confirmDelete = (type: 'cert' | 'staff', item: any) => {
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        if (type === 'cert') setCertificates(prev => prev.filter(c => c.id !== item.id));
        else setStaffList(prev => prev.filter(s => s.id !== item.id));
                ...
                {certificates.map(cert => (
                  <tr key={cert.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      ...
                      <button onClick={() => confirmDelete('cert', cert)} className="p-2 text-slate-400 hover:text-rose-500 transition-colors"><Trash2 size={16} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
                ...
                {staffList.map(staff => (
                  <tr key={staff.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-bold text-slate-700 dark:text-white">{staff.name}</p>
                    ...
                    <td className="px-6 py-4 text-right">
                      ...
                      <button onClick={() => confirmDelete('staff', staff)} className="p-2 text-slate-400 hover:text-rose-500 transition-colors"><Trash2 size={16} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-950 w-full max-w-md rounded-[2rem] p-8 text-center animate-in zoom-in-95">
            ...
            <p className="text-slate-500 mb-6">You are about to delete <span className="font-bold text-slate-900 dark:text-white">{deleteTarget?.item.name}</span>. This cannot be undone.</p>
...
const CertModal = ({ isEdit, onClose, formData, setFormData, subOptions, setSubOptions, subQuestions, setSubQuestions, onSubmit }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-950 w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col max-h-[90vh] overflow-hidden">
        ...
        <form onSubmit={onSubmit} className="p-6 overflow-y-auto space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="text-xs font-bold text-slate-400 uppercase">Certificate Name</label>
              <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 outline-none" />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase">Base Fee (₱)</label>
              <input required type="number" value={formData.fee} onChange={e => setFormData({...formData, fee: Number(e.target.value)})} className="w-full px-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 outline-none" />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase">Processing Days</label>
              <input required type="number" value={formData.days} onChange={e => setFormData({...formData, days: Number(e.target.value)})} className="w-full px-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 outline-none" />
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center border-b pb-2 dark:border-slate-800">
              ...
              <button type="button" onClick={addOption} className="text-xs text-emerald-600 font-bold hover:underline">+ Add Row</button>
            </div>
            {subOptions.map((opt: any) => (
              <div key={opt.id} className="flex gap-2">
                <input required type="text" value={opt.name} onChange={e => setSubOptions(subOptions.map((o:any) => o.id === opt.id ? {...o, name: e.target.value} : o))} placeholder="Option Name" className="flex-1 px-4 py-2 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 outline-none" />
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center border-b pb-2 dark:border-slate-800">
              ...
              <button type="button" onClick={addQuestion} className="text-xs text-indigo-600 font-bold hover:underline">+ Add Row</button>
            </div>
            {subQuestions.map((q: any) => (
              <div key={q.id} className="flex gap-2">
                <input required type="text" value={q.question} onChange={e => setSubQuestions(subQuestions.map((sq:any) => sq.id === q.id ? {...sq, question: e.target.value} : sq))} placeholder="Label" className="flex-1 px-4 py-2 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 outline-none" />
                <select value={q.type} onChange={e => setSubQuestions(subQuestions.map((sq:any) => sq.id === q.id ? {...sq, type: e.target.value} : sq))} className="w-24 px-2 py-2 rounded-lg bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 outline-none">
          ...
          <div className="flex gap-3 pt-6 border-t dark:border-slate-800">
            ...
            <button type="submit" className="flex-1 bg-emerald-500 text-white py-2.5 rounded-xl font-bold shadow-lg shadow-emerald-500/20 active:scale-95">Save Certificate</button>
          </div>
        </form>
      </div>
    </div>
...
const StaffModal = ({ isEdit, onClose, formData, setFormData, onSubmit }: any) => {
    ...
    const hasTypedConfirm = formData.password_confirmation.length > 0;

    return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
        <div className="bg-white dark:bg-slate-950 w-full max-w-md rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 animate-in zoom-in-95">
        <div className="px-6 py-4 border-b dark:border-slate-800 flex items-center justify-between">
            <h3 className="text-xl font-bold">{isEdit ? 'Update Staff Member' : 'Invite New Staff'}</h3>
            <button onClick={onClose} className="p-2 rounded-xl text-slate-500"><X size={20} /></button>
        </div>
        <form onSubmit={onSubmit} className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="text-xs font-bold text-slate-400 uppercase px-1">Role</label>
                    <select value={formData.role} onChange={e => setFormData({...formData, role: e.target.value})} className="w-full px-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 outline-none">
                ...
                <div>
                    <label className="text-xs font-bold text-slate-400 uppercase px-1">Username</label>
                    <input required type="text" value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} className="w-full px-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 outline-none" />
                </div>
            </div>
            <div>
                <label className="text-xs font-bold text-slate-400 uppercase px-1">Email</label>
                <input required type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full px-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 outline-none" />
            </div>
            
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="text-xs font-bold text-slate-400 uppercase px-1">{isEdit ? 'New Pwd' : 'Password'}</label>
                    <input 
                        required={!isEdit} 
                        type="password" 
                        value={formData.password} 
                        onChange={e => setFormData({...formData, password: e.target.value})} 
                        className="w-full px-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 outline-none focus:border-indigo-500" 
                    />
                </div>
                <div>
                    <label className="text-xs font-bold text-slate-400 uppercase px-1">Confirm</label>
                    <input 
                        required={!isEdit || formData.password !== ''} 
                        type="password" 
                        value={formData.password_confirmation} 
                        onChange={e => setFormData({...formData, password_confirmation: e.target.value})} 
                        className={`w-full px-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-900 border outline-none transition-colors ${
                            !isPasswordMatch && hasTypedConfirm 
            ...
            {!isPasswordMatch && hasTypedConfirm && (
                <p className="text-[10px] text-rose-500 font-bold uppercase tracking-tight px-1 italic animate-in fade-in slide-in-from-top-1">
            ...
            <div className="flex gap-3 pt-4 border-t dark:border-slate-800">
                ...
                <button 
                    type="submit" 
                    disabled={(!isPasswordMatch && !isEdit) || (isEdit && formData.password !== '' && !isPasswordMatch)}
                    className="flex-1 bg-indigo-600 disabled:bg-slate-300 disabled:cursor-not-allowed text-white py-2.5 rounded-xl font-bold shadow-lg shadow-indigo-600/20 active:scale-95"
                >
                    Save Staff
                </button>
            </div>
