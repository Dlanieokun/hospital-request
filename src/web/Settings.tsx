...
const SettingsSetup = () => {
  ...
  const handleGoToMapping = async () => {
    ...
    navigate('/data-mapping', { 
      state: { 
        certificate: { ...certFormData, sub_questions: subQuestions, sub_options: subOptions },
        patient: { datas }
      }
    });
  };
  ...
  return (
    <div className="p-8 space-y-8 relative bg-slate-50 min-h-screen">
      <div className="fixed top-8 right-8 z-[110] flex flex-col gap-3 pointer-events-none">
        {toasts.map(toast => (
            <div key={toast.id} className={`flex items-center gap-3 px-6 py-4 rounded-2xl shadow-2xl border animate-in slide-in-from-right-full pointer-events-auto bg-white ${toast.type === 'success' ? 'border-emerald-100 text-emerald-600' : 'border-rose-100 text-rose-500'}`}>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <h2 className="text-3xl font-black text-slate-900 tracking-tight">Configuration Center</h2>
        </div>

        <div className="flex p-1.5 bg-slate-100 rounded-2xl border border-slate-200">
          <button onClick={() => setActiveTab('certificates')} className={`flex items-center gap-2 px-6 py-2 rounded-xl font-bold transition-all ${activeTab === 'certificates' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-500'}`}>
            <FileText size={18} /> Certificates
          </button>
          <button onClick={() => setActiveTab('staff')} className={`flex items-center gap-2 px-6 py-2 rounded-xl font-bold transition-all ${activeTab === 'staff' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}>
      ...
      <div className="flex flex-col md:flex-row items-center gap-4 justify-between">
        <div className="relative w-full md:w-96">
            ...
            <input 
                ...
                className="w-full pl-11 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all"
            />
        </div>
            ...
            {activeTab === 'certificates' ? (
                (paginatedData as Certificate[]).map(cert => (
                    <tr key={cert.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-6 py-4 font-bold text-slate-700">{cert.name}</td>
                ...
                (paginatedData as Staff[]).map(staff => (
                    <tr key={staff.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-6 py-4 flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center font-bold text-slate-500 border border-slate-200">{staff.name.charAt(0)}</div>
                            <div><p className="font-bold text-slate-700">{staff.name}</p><p className="text-xs text-slate-400">@{staff.username}</p></div>
                        </td>
                        <td className="px-6 py-4">
                            <span className="px-3 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-black rounded-lg uppercase tracking-widest">{staff.role}</span>
                        </td>
                        <td className="px-6 py-4 text-right">
        ...
        {totalPages > 1 && (
            <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                <p className="text-sm text-slate-500 font-medium">
                    Showing <span className="text-slate-900">{(currentPage - 1) * ITEMS_PER_PAGE + 1}</span> to <span className="text-slate-900">{Math.min(currentPage * ITEMS_PER_PAGE, filteredData.length)}</span> of <span className="text-slate-900">{filteredData.length}</span> results
                </p>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-[2rem] p-8 text-center animate-in zoom-in-95">
            ...
            <h3 className="text-xl font-black mb-2 text-slate-900">Are you sure?</h3>
            <p className="text-slate-500 mb-6">Delete <span className="font-bold text-slate-900">{deleteTarget?.item.name}</span>? This is permanent.</p>
...
const CertModal = ({ isEdit, onClose, formData, setFormData, subOptions, setSubOptions, subQuestions, setSubQuestions, onSubmit, onSync }: any) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border flex flex-col max-h-[90vh] overflow-hidden">
          ...
          <div className="flex items-center gap-1">
            {isEdit && (
                <button 
                    type="button"
                    onClick={onSync}
                    className="p-2 rounded-xl text-slate-500 hover:bg-slate-100 hover:text-emerald-600 transition-all active:rotate-180 duration-500"
                    title="Configure Data Mapping"
                >
                    <Link2 size={20} />
                </button>
            )}
            <button onClick={onClose} className="p-2 rounded-xl text-slate-500 hover:bg-slate-100 transition-colors">
                <X size={20} />
            </button>
          </div>
        </div>
        ...
        <form onSubmit={onSubmit} className="p-6 overflow-y-auto space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 outline-none" />
            </div>
            
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase">Fee (₱)</label>
              <input required type="number" value={formData.fee} onChange={e => setFormData({...formData, fee: Number(e.target.value)})} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 outline-none" />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase">Days</label>
              <input required type="number" value={formData.days} onChange={e => setFormData({...formData, days: Number(e.target.value)})} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 outline-none" />
            </div>

            <div className="col-span-2">
              <label className="text-xs font-bold text-slate-400 uppercase">URL</label>
              <input type="text" value={formData.url} onChange={e => setFormData({...formData, url: e.target.value})} placeholder="https://..." className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 outline-none" />
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {subOptions.map((opt: any) => (
              <div key={opt.id} className="flex gap-2">
                <input required type="text" value={opt.name} onChange={e => setSubOptions(subOptions.map((o: any) => o.id === opt.id ? {...o, name: e.target.value} : o))} placeholder="Option Name" className="flex-1 px-4 py-2 rounded-lg bg-slate-50 border border-slate-200 outline-none" />
          ...
          <div className="space-y-4">
            ...
            {subQuestions.map((q: any) => (
              <div key={q.id} className="flex gap-2">
                <input required type="text" value={q.question} onChange={e => setSubQuestions(subQuestions.map((sq: any) => sq.id === q.id ? {...sq, question: e.target.value} : sq))} placeholder="Label" className="flex-1 px-4 py-2 rounded-lg bg-slate-50 border border-slate-200 outline-none" />
                <select value={q.type} onChange={e => setSubQuestions(subQuestions.map((sq: any) => sq.id === q.id ? {...sq, type: e.target.value} : sq))} className="w-24 px-2 py-2 rounded-lg bg-slate-50 border border-slate-200 outline-none">
...
const StaffModal = ({ isEdit, onClose, formData, setFormData, onSubmit }: any) => {
    ...
    return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
        <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl border animate-in zoom-in-95">
        <div className="px-6 py-4 border-b flex items-center justify-between">
        ...
        <form onSubmit={onSubmit} className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select value={formData.role} onChange={e => setFormData({...formData, role: e.target.value})} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 outline-none">
                ...
                <div>
                    ...
                    <input required type="text" value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 outline-none" />
                </div>
            </div>
            <div>
                <label className="text-xs font-bold text-slate-400 uppercase">Email</label>
                <input required type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 outline-none" />
            </div>
            
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="text-xs font-bold text-slate-400 uppercase">{isEdit ? 'New Pwd' : 'Password'}</label>
                    <input required={!isEdit} type="password" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 outline-none" />
                </div>
                <div>
                    <label className="text-xs font-bold text-slate-400 uppercase">Confirm</label>
                    <input required={!isEdit || formData.password !== ''} type="password" value={formData.password_confirmation} onChange={e => setFormData({...formData, password_confirmation: e.target.value})} className={`w-full px-4 py-2 rounded-xl bg-slate-50 border outline-none ${!isPasswordMatch && formData.password_confirmation ? 'border-rose-500' : 'border-slate-200'}`} />
                </div>
            </div>
