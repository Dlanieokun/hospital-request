...
import { Plus, Edit2, Trash2, FileText, X, PlusCircle, Calendar, HelpCircle } from 'lucide-react';

const SettingsSetup = () => {
  // --- Data States ---
  const [certificates, setCertificates] = useState([
    { id: 1, name: 'Medical Certificate', price: '₱150.00', status: 'Active', days: '2', subOptions: [], subQuestions: [] },
    { id: 2, name: 'Birth Certificate', price: '₱200.00', status: 'Active', days: '5', subOptions: [], subQuestions: [] },
  ]);
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800">
              ...
              <th className="px-6 py-4 text-xs font-bold uppercase text-slate-500">Processing Time</th>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {certificates.map((cert) => (
              <tr key={cert.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                ...
                <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{cert.days} Days</td>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-950 w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col max-h-[90vh]">
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-950 sticky top-0 z-10">
          ...
          <button onClick={onClose} className="p-1 hover:bg-slate-100 rounded-lg text-slate-500 transition-colors"><X size={20} /></button>
        </div>
        ...
        <form onSubmit={onSubmit} className="p-6 overflow-y-auto space-y-8">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Processing Time (Days)</label>
                <div className="relative">
                  <input 
                    required type="number" value={formData.days} 
                    onChange={(e) => setFormData({...formData, days: e.target.value})} 
                    placeholder="e.g. 3" 
                    className="w-full px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-transparent outline-none focus:ring-2 focus:ring-emerald-500/20 pl-10" 
                  />
